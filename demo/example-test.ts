...
module('Browser', () => {
  ...
  test('can build and query a DOM tree', (assert) => {
    ...
    (['Alice', 'Bob', 'Carol'] as string[]).forEach((name) => {
...
module('TypeScript', () => {
  ...
  test('typed interfaces work without tsconfig', (assert) => {
    const user: User = { id: 1, name: 'Alice', role: 'admin' };
    assert.equal(greet(user), 'Hello, Alice (admin)');
  });

  test('type narrowing works', (assert) => {
    const value: string | number = 42;
    assert.ok(typeof value === 'number');
    assert.equal((value as number) * 2, 84);
  });
});
